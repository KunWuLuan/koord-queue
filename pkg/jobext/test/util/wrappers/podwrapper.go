...
func (p *PodWrapper) Res(resMap map[v1.ResourceName]string) *PodWrapper {
	...
	name := fmt.Sprintf("con%d", len(p.Spec.Containers))
	p.Spec.Containers = append(p.Spec.Containers, MakeContainer().Name(name).Image("pause").Resources(resMap).Obj())
	return p
}
...
func (p *PodWrapper) Req(reqMap map[v1.ResourceName]string) *PodWrapper {
	...
	name := fmt.Sprintf("con%d", len(p.Spec.Containers))
	p.Spec.Containers = append(p.Spec.Containers, MakeContainer().Name(name).Image("pause").ResourceRequests(reqMap).Obj())
	return p
}
...
func (p *PodWrapper) Lim(limMap map[v1.ResourceName]string) *PodWrapper {
	...
	name := fmt.Sprintf("con%d", len(p.Spec.Containers))
	p.Spec.Containers = append(p.Spec.Containers, MakeContainer().Name(name).Image("pause").ResourceLimits(limMap).Obj())
	return p
}
...
func (p *PodWrapper) InitReq(resMap map[v1.ResourceName]string) *PodWrapper {
	...
	p.Spec.InitContainers = append(p.Spec.InitContainers, MakeContainer().Name(name).Image("pause").Resources(resMap).Obj())
	return p
}
...
func (p *PodWrapper) SidecarReq(resMap map[v1.ResourceName]string) *PodWrapper {
	...
	p.Spec.InitContainers = append(p.Spec.InitContainers, MakeContainer().Name(name).Image("pause").RestartPolicy(v1.ContainerRestartPolicyAlways).Resources(resMap).Obj())
	return p
}

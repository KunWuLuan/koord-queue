...
func TestAddQueueUnit_CheckUsage_WithGpu(t *testing.T) {
	{
		q := &v1alpha1.ElasticQuota{}
		q.Spec = v1alpha1.ElasticQuotaSpec{}
		q.Spec.Max = make(v1.ResourceList)
		q.Spec.Min = make(v1.ResourceList)
		q.Spec.Max["cpu"] = resource.MustParse("15")
		q.Spec.Max[v1.ResourceName("nvidia.com/gpu")] = resource.MustParse("200")
		q.Spec.Min["cpu"] = resource.MustParse("10")
		q.Spec.Min[v1.ResourceName("nvidia.com/gpu")] = resource.MustParse("100")
		q.Annotations = map[string]string{
			"aa": "bb",
		}
		...
		queueUnit1.Unit.Spec.Resource["cpu"] = resource.MustParse("5")
		queueUnit1.Unit.Spec.Resource["mem"] = resource.MustParse("5")
		queueUnit1.Unit.Spec.Resource[v1.ResourceName("nvidia.com/gpu")] = resource.MustParse("1")
		elasticQuotaInfo.AddQueueUnit("q1", queueUnit1)
		...
		queueUnit2.Unit.Spec = api.QueueUnitSpec{}
		queueUnit2.Unit.Spec.Resource = make(v1.ResourceList)
		queueUnit2.Unit.Spec.Resource["cpu"] = resource.MustParse("1")
		queueUnit2.Unit.Spec.Resource["mem"] = resource.MustParse("500")
		queueUnit2.Unit.Spec.Resource[v1.ResourceName("nvidia.com/gpu")] = resource.MustParse("1")
		err := elasticQuotaInfo.CheckUsage("", queueUnit2, 1)
		assert.True(t, err == nil)

		queueUnit2.Unit.Spec.Resource[v1.ResourceName("nvidia.com/gpu")] = resource.MustParse("2")
		err = elasticQuotaInfo.CheckUsage("", queueUnit2, 1)
		assert.True(t, err != nil)
	}
	{
		...
		q.Spec.Min = make(v1.ResourceList)
		q.Spec.Max["cpu"] = resource.MustParse("15")
		q.Spec.Max[v1.ResourceName("nvidia.com/gpu")] = resource.MustParse("400")
		q.Spec.Min["cpu"] = resource.MustParse("10")
		q.Spec.Min[v1.ResourceName("nvidia.com/gpu")] = resource.MustParse("300")
		q.Annotations = map[string]string{
			"aa": "bb",
		}
		...
		elasticQuotaInfo.SelfGuaranteedUsed[v1.ResourceName("nvidia.com/gpu")] = 1
		...
		queueUnit2.Unit.Spec.Resource["mem"] = resource.MustParse("500")
		queueUnit2.Unit.Spec.Resource[v1.ResourceName("nvidia.com/gpu")] = resource.MustParse("1")
		err := elasticQuotaInfo.CheckUsage("q2", queueUnit2, 1)
		assert.True(t, err == nil)

		queueUnit2.Unit.Spec.Resource[v1.ResourceName("nvidia.com/gpu")] = resource.MustParse("3")
		err = elasticQuotaInfo.CheckUsage("", queueUnit2, 1)
		assert.True(t, err != nil)
	...
	}
	{
		...
		q.Spec.Min = make(v1.ResourceList)
		q.Spec.Max["cpu"] = resource.MustParse("15")
		q.Spec.Max[v1.ResourceName("nvidia.com/gpu")] = resource.MustParse("300")
		q.Spec.Min["cpu"] = resource.MustParse("10")
		q.Spec.Min[v1.ResourceName("nvidia.com/gpu")] = resource.MustParse("200")
		q.Annotations = map[string]string{
			"aa": "bb",
		}
...
func TestAddQueueUnit_CheckUsage_WithGpu_Parent(t *testing.T) {
	q := &v1alpha1.ElasticQuota{}
	q.Name = "q1"
	q.Spec = v1alpha1.ElasticQuotaSpec{}
	q.Spec.Max = make(v1.ResourceList)
	...
	q2 := q.DeepCopy()
	q2.Spec.Min[v1.ResourceName("nvidia.com/gpu")] = resource.MustParse("100")
	q2.Name = "q2"
	q2.Labels = map[string]string{
		ParentQuotaNameLabelKey: "q1",
	}

	q1Info := NewElasticQuotaInfo(q)
	assert.True(t, q1Info.Reserved != nil)
	assert.Equal(t, int64(15), q1Info.Max["cpu"])
	assert.Equal(t, int64(10), q1Info.Min["cpu"])
	...
	queueUnit1.Unit.Spec.Resource[v1.ResourceName("nvidia.com/gpu")] = resource.MustParse("1")
	queueUnit1.Unit.Labels = map[string]string{
		QuotaNameLabelKey: "q2",
	}
	q1Info.AddQueueUnit("q1", queueUnit1)
	q2Info := NewElasticQuotaInfo(q2)
	q2Info.AddQueueUnit("q2", queueUnit1)

	assert.Equal(t, int64(100), q1Info.Used[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(0), q1Info.SelfUsed[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(0), q1Info.SelfGuaranteedUsed[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(100), q1Info.ChildrenUsed[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(100), q1Info.ChildrenGuaranteedUsed[v1.ResourceName("nvidia.com/gpu")])

	assert.Equal(t, int64(100), q2Info.Used[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(100), q2Info.SelfUsed[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(100), q2Info.SelfGuaranteedUsed[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(0), q2Info.ChildrenUsed[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(0), q2Info.SelfUsed[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(0), q2Info.ChildrenGuaranteedUsed[v1.ResourceName("nvidia.com/gpu")])
	...
	// if queue unit belongs to child, can't deque
	queueUnit2.Unit.Labels = map[string]string{
		QuotaNameLabelKey: "q2",
	}
	err := q2Info.CheckUsage("q2", queueUnit2, 1)
	assert.True(t, err != nil)

	// if queue unit belongs to parent, can deque
	queueUnit2.Unit.Labels[QuotaNameLabelKey] = "q1"
	err = q1Info.CheckUsage("q1", queueUnit2, 1)
	assert.True(t, err == nil)

	q1Info.DeleteQueueUnit("q1", queueUnit1)
	q2Info.DeleteQueueUnit("q2", queueUnit1)
	assert.Equal(t, int64(0), q2Info.SelfUsed[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(0), q2Info.SelfUsed[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(0), q1Info.SelfUsed[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(0), q2Info.Used[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(0), q1Info.Used[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(0), q2Info.ChildrenUsed[v1.ResourceName("nvidia.com/gpu")])
	assert.Equal(t, int64(0), q1Info.ChildrenUsed[v1.ResourceName("nvidia.com/gpu")])
